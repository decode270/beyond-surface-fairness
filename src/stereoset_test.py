...
def load_model_and_tokenizer(args):
    ...
    if not args.use_adapter:
        ...
        return base, tok
    print(f"[INFO] 正在尝试从以下路径加载 Adapter: {args.adapter_dir}")
    ...
    if not is_lora_active:
        print("\n" + "!"*60)
        print("错误：Adapter 加载失败！模型层依然是普通的 Linear。")
        print(f"请检查路径 {args.adapter_dir} 下是否存在 adapter_model.bin 或 adapter_model.safetensors")
        print("!"*60 + "\n")
        # 强制停止，防止浪费时间测出错误结果
        raise RuntimeError("LoRA Adapter failed to inject into the model layers.")
    
    
    val_before = peft_model.base_model.model.model.layers[0].self_attn.q_proj.weight[0, 0].item() # 此时是 base_model 里的原始值
    merged = peft_model.merge_and_unload().eval()
    val_after = merged.model.layers[0].self_attn.q_proj.weight[0, 0].item()
    print(f"[DEBUG] 合并前测试点数值: {val_before}")
    print(f"[DEBUG] 合并后测试点数值: {val_after}")
    ...
    if abs(val_before - val_after) < 1e-9:
        print("！！！最终结论：权重完全没有变化。Adapter 加载失败。！！！")
    else:
        print("--- 成功：权重已发生变化 ---")
    return merged, tok
